...
| 📦 资料包 | `kd ask` 检索+深读 topK 全文一站带回,附 **chunk 命中段落**(可引用 `[n](chunk#m)`),交当前 AI 合成带引用回答 |
| 🤖 AI 合成 | `kd ai` 关键词规划→检索→按 ANSWER-SPEC 合成,接你自己的模型通道,自动降级 |
| 💬 分享对话 | 读官方 AI 分享对话全文(含引用来源),可作官方效果样本与评测金标 |
| 📚 corpus 语料 | 深读/发现层成果落 `~/.lingeebuild/corpus`(一文档一 md,带原链接 front-matter),**agent 用 rg 直接检索**(零上游),stub 按需深读覆盖 |
| 🗄️ 上游缓存 | **opt-in**(本机 `KSEARCH_INDEX=on` 开):检索/深读自动沉淀本地 sqlite(永久),重复查询 **≈30 倍提速**、上游调用归零;v5 缩编为纯上游缓存(FTS5/local 冻结,ADR-0004) |
| 🧭 发现层 | `scripts/discovery_sweep.py` 时间网格(词表×时间倒序,手动触发,1 请求/秒、单轮 ≤200),把索引外新/更新文档带进 corpus |
| 🧪 评测 | `data/eval` 26 例双层金标(**usage 正式口径 12 例**/reference 参考 14 例)+ `scripts/run_eval.py` A/B(recall@K/MRR/时延),检索改动先过评测 |
